...
bool fd_system_dispatch_filter(const fd_envelope_t *envelope) {
    return
        (envelope->system == fd_envelope_system_firefly) &&
        (envelope->subsystem == fd_envelope_subsystem_system);
}

void fd_system_dispatch_initialize(void) {
    fd_assert_log_initialize();

    fd_dispatch_add_process(fd_system_dispatch_process, fd_system_dispatch_filter);
}