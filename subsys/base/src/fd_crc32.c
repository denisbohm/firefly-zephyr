...
// The values in this table are those used in the Ethernet CRC algorithm.
static const uint32_t fd_crc32_table[] = {
...
void fd_crc32_update(fd_crc32_t *crc32, const uint8_t *data, size_t length) {
    ...
    while (length--) {
        uint8_t c = *data++ & 0xff;
        crc = (crc << 8) ^ fd_crc32_table[(crc >> 24) ^ c];
    }
    crc32->crc = crc;
}
...
void fd_crc32_finalize(fd_crc32_t *crc32) {
    ...
    // pad with zeroes
    uint32_t crc = crc32->crc;
    uint32_t count = crc32->count;
    for (uint32_t i = count % 4; i < 4; i++) {
        crc = (crc << 8) ^ fd_crc32_table[(crc >> 24) ^ 0];
    }
    crc32->crc = crc;
}
