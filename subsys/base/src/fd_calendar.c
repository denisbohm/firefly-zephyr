...
static int leap(int32_t y) {
    return (((y % 4) == 0) && ((y % 100) != 0)) || ((y % 400) == 0);
}

/* Number of leap years from 1970 to "y" (not including "y" itself). */
static int nleap(int32_t y) {
    return (y - 1969) / 4 - (y - 1901) / 100 + (y - 1601) / 400;
}

/* Accumulated number of days from 01-Jan up to start of current month. */
static int32_t ydays(int m) {
    switch (m) {
        case 0: return 0;
        case 1: return 31;
        case 2: return 59;
...
fd_calendar_12_hour_t fd_calendar_get_12_hour(const fd_calendar_t *date_time) {
    fd_calendar_12_hour_t calendar_12_hour;
    if (date_time->hour == 0) {
        calendar_12_hour.meridiem = 'a';
        calendar_12_hour.hour = 12;
    } else
    if (date_time->hour < 12) {
        calendar_12_hour.meridiem = 'a';
        calendar_12_hour.hour = date_time->hour;
    } else
    if (date_time->hour == 12) {
        calendar_12_hour.meridiem = 'p';
        calendar_12_hour.hour = 12;
    } else
    if (date_time->hour < 24) {
        calendar_12_hour.meridiem = 'p';
        calendar_12_hour.hour = date_time->hour - 12;
    } else {
        calendar_12_hour.meridiem = 'a';
        calendar_12_hour.hour = 0;
    }
    return calendar_12_hour;
}
