...
static uint8_t fd_sdcard_read_r1() {
    ...
    fd_assert_fail("sd r1 not received");
    return 0xff;
}

static uint8_t fd_sdcard_read_r7(uint32_t *data) {
    uint8_t r1 = fd_sdcard_read_r1();
    fd_assert((r1 & ~fd_sdcard_r1_in_idle_state) == 0);
    uint8_t bytes[4];
...
static uint8_t fd_sdcard_send_cmd0(void) {
    ...
    uint8_t r1 = fd_sdcard_read_r1();
    fd_assert(r1 == fd_sdcard_r1_in_idle_state);
    return r1;
}
...
static void fd_sdcard_send_cmd55(void) {
    ...
    uint8_t r1 fd_unused = fd_sdcard_read_r1();
//    fd_assert(r1 == 0x01);
}
...
void fd_sdcard_erase(uint32_t address, size_t length) {
    ...
    fd_assert(r1 == 0);
    fd_sdcard_spi_dummy_uint8();
    fd_sdcard_send_cmd(33, (uint32_t)(address + length), 0x00);
...
void fd_sdcard_read_block(uint32_t address, uint8_t *data) {
    ...
    for (int i = 0; i < 1024; ++i) {
        ...
        uint16_t crc fd_unused = fd_sdcard_spi_rx_uint16();
        return;
    }
}
...
void fd_sdcard_write_multiple_blocks_end(void) {
    ...
    uint8_t response fd_unused = fd_sdcard_spi_rx_uint8();
    ...
    uint8_t r1 fd_unused = fd_sdcard_send_acmd22(&well_written_block_count);
    fd_assert(well_written_block_count == fd_sdcard.write_multiple_blocks_count);
...
void fd_sdcard_initialize(fd_sdcard_spi_t spi) {
    ...
    fd_assert(fd_sdcard.csd.erase_blk_en);
}
