...
bool fd_watch_screen_powering_off_button(const fd_button_event_t *event) {
    if (fd_button_was_pressed(event, fd_watch_button_both)) {
        fd_ux_t *ux = fd_ux_get(0);
        fd_ux_set_screen(ux, fd_watch_screen_id_time);
    }
    return false;
}
